...
ggplot(finalDF) + geom_point(aes(x = `Population`, y = `Violent Crime`, color = "Violent Crime")) + geom_smooth(se = FALSE, aes(x = `Population`, y = `Violent Crime`, color = "Violent Crime"), method = "lm", formula = y ~ x) + 
  ...
  geom_smooth(se = FALSE, aes(x = `Population`, y = `Property Crime`, color = "Property Crime"), method = "lm", formula = y ~ x) + ylab("Crimes per 100k people") + xlab("County Population") + ggtitle("Wisconsin crimes by county population", "2019")
```
